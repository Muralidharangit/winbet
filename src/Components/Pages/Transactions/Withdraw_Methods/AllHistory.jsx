...
const DepositHistory = () => {
  ...
  useEffect(() => {
    ...
    if (next <= (data.last_page || 1)) {
      qc.prefetchQuery({
        ...
        queryFn: () =>
          fetchWithdrawHistory({ page: next, perPage: ITEMS_PER_PAGE, token }),
        staleTime: 5 * 60 * 1000,
      });
    }
    if (prev >= 1) {
      qc.prefetchQuery({
        ...
        queryFn: () =>
          fetchWithdrawHistory({ page: prev, perPage: ITEMS_PER_PAGE, token }),
        staleTime: 5 * 60 * 1000,
      });
    }
  ...
  const prefetchPage = (p) => {
    ...
    qc.prefetchQuery({
      ...
      queryFn: () =>
        fetchWithdrawHistory({ page: p, perPage: ITEMS_PER_PAGE, token }),
      staleTime: 5 * 60 * 1000,
    });
  };
